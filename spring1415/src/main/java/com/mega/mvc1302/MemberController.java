...
public class MemberController {
	...
	public void list(Model model) { //입력값 없어서 dto안넣어도 됨. 다 갖고올 것!
		//model은 써줘야하는데 views까지 값 가져가 출력해야하니까.
		//select하는 경우에는 model을 꼭 넣어줄 것!
	...
	public String del(MemberDTO memberDTO) {
		...
		List<MemberDTO> list = dao.list();
		return "list";
		//views로 안감. db처리 때문에 .jsp가 아니라 주소를 redirect
	...
	public void check2(MemberDTO memberDTO, Model model) {
		...
		String result = "중복된 id가 없습니다. 사용가능합니다.";
		if (dto != null) { //null이 아니면 검색 결과가 있는거니까 중복 있는것
			result = "중복된 id가 있습니다. 사용불가능합니다.";
		}
